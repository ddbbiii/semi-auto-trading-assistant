...
| 情况 | 系统行为 |
| --- | --- |
| 未主动上报持仓变化 | 最近一次已确认持仓持续作为计算基线，不因时间流逝要求重复同步 |
| 主动上报交易或导入新快照 | 更新持仓基线后重新检查组合风险 |
...
完整分析会同时展示一份由后端生成的数据覆盖清单。持仓数量、成本和绝对市值继续留在本地规则层；模型只接收代码、持仓内权重、可选账户仓位、成本收益率、涨跌幅和用户规则等派生信息。模型不能把未发送的原始金额误报成“用户没有同步”。

- 券商未提供逐标的盈亏时，系统按已确认成本和最新行情在本地估算，并明确标注“按成本估算”。
- 行情源未直接给出涨跌幅时，系统可用最新价与昨收在本地派生，并记录派生来源。
- 未同步账户净资产与现金时，只计算“已投资持仓内部权重”，不会用它冒充账户实际仓位或据此执行目标仓位规则。
- 未同步可卖数量时，即使触发硬退出语义，也不会生成具体卖出数量或订单草案。
