...
4. **检查官方披露**：美股查询 SEC EDGAR，A 股查询巨潮资讯，港股查询港交所披露易；单一来源失败会明确降级。
5. **执行规则**：仓位、集中度、数据过期和用户确认的风险条件由确定性代码计算。
6. **生成解释**：可选的 OpenAI-compatible 模型只解释公开证据，不能改变价格、仓位、数量、优先级或有效期。
7. **进入收件箱**：只有数据和规则都满足要求的事项才可操作；其余明确显示“需要核验”或“不可操作”。

## 投资决策政策

设置页公开展示系统采用的结构化政策，而不是把判断隐藏在提示词中。该全局政策默认覆盖全部持仓，不要求逐只股票重复配置。逐标的档案只是可选覆盖，用于记录某只股票特有的论文、目标仓位、价格复核线或退出条件。
...
- 官方来源查询成功但近期没有文件时，显示为“已检查、0 条文件”，不会误报成接口缺失；查询失败的标的单独显示降级原因。
...
- SEC EDGAR、巨潮资讯、港交所披露易：按市场采集近期官方公告与财报，并保存来源链接。
